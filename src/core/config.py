...
HOME_DESKTOP_SCRIPT_PATH = path.normpath(path.join(HOME_CONFIGURATION_DIR, settings.DEFAULT_DESKTOP_SCRIPT_FILENAME))
...
DEFAULT_DESKTOP_SCRIPT_PATH = path.normpath(
    path.join(SRC_CONFIGURATION_DIR, settings.DEFAULT_DESKTOP_SCRIPT_FILENAME)
)
...
def _ensure_desktop_script() -> None:
    if path.isfile(HOME_DESKTOP_SCRIPT_PATH):
        return
    if not path.isdir(HOME_CONFIGURATION_DIR):
        makedirs(HOME_CONFIGURATION_DIR)
    try:
        shutil.copy2(DEFAULT_DESKTOP_SCRIPT_PATH, HOME_DESKTOP_SCRIPT_PATH)
        logging.info(f"Created default desktop script at {HOME_DESKTOP_SCRIPT_PATH}")
    except OSError:
        logging.error(
            f"Failed to create desktop script at {HOME_DESKTOP_SCRIPT_PATH}. "
            f"Source was {DEFAULT_DESKTOP_SCRIPT_PATH}."
        )
