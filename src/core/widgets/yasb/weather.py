    ...
    def _popup_card(self):
        ...
        if buttons_config["enabled"] and self._weather_card["show_hourly_forecast"]:
            ...
            for data_type, icon in button_configs:
                ...
                tooltip_label = {"temperature": "温度", "rain": "降雨", "snow": "降雪"}.get(data_type, data_type)
                set_tooltip(btn, tooltip_label, delay=400, position="top")
        ...
        today_label1 = QLabel(
            ...
            f"气压 {self._weather_data['{pressure}']} - 能见度 {self._weather_data['{vis}']} - 云量 {self._weather_data['{cloud}']}%\n"
            f"降雨概率 {self._weather_data['{daily_chance_of_rain}']} - 降雪概率 {self._weather_data['{daily_chance_of_snow}']}"
        )
        ...
        today_label2 = QLabel(
            ...
            f"{'<br>预警结束 ' + self._weather_data['{alert_end_date}'] if self._weather_data['{alert_end_date}'] else ''}"
        ...
        if self._tooltip:
            ...
            if rain != "N/A" and snow != "N/A" and (int(rain.rstrip("%")) > 0 or int(snow.rstrip("%")) > 0):
                ...
                if int(rain.rstrip("%")) > 0:
                    precip.append(f"雨 {rain}")
                if int(snow.rstrip("%")) > 0:
                    precip.append(f"雪 {snow}")
    ...
    def process_weather_data(self, weather_data: dict[str, Any]):
        try:
            ...
            self._weather_data = {
                ...
                "{is_day}": "白天" if current["is_day"] else "夜间",
                # Icons
            ...
            if self._weather_data is None:
                self._weather_data = {
                    ...
                    "{min_temp}": "无",
                    "{max_temp}": "无",
                    "{daily_chance_of_rain}": "无",
                    "{daily_chance_of_snow}": "无",
                    "{hourly_chance_of_rain}": "无",
                    "{hourly_chance_of_snow}": "无",
                    "{location}": "无",
                    "{location_region}": "无",
                    "{location_country}": "无",
                    "{time_zone}": "无",
...
def get_weather(code: int, day: bool) -> tuple[str, str]:
    ...
    if code in foggy_codes:
        return f"foggy{time}", "有雾"
    if code in rainy_codes:
        return f"rainy{time}", "下雨"
    if code in snowy_codes:
        return f"snowy{time}", "下雪"
    if code in thunderstorm_codes:
        return f"thunderstorm{time}", "雷雨"
