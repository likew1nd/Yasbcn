...
_DOWNLOAD_READY_TEXT = "下载并安装"
_DOWNLOADING_TEXT = "正在下载..."

_CLOSE_BUTTON_TEXT = "关闭"
...
class ReleaseFetcher(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.error.emit("无法连接到 GitHub。请检查网络连接后重试。")
...
class DownloadWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if bytes_read == 0:
                raise IOError("未从服务器收到数据。")
            if total_size and bytes_read < total_size:
                raise IOError("下载未完成，连接已中断。")
...
class UpdateDialog(QDialog):
    ...
    def set_release_info(self, release_info: ReleaseInfo) -> None:
        ...
        if update_service._current_channel == "dev":
            version_display = f"新的 Dev 构建（{release_info.version.replace('dev-', '')}）"
        else:
