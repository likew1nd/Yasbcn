...
class TrafficWidget(BaseWidget):
    ...
    def _get_shared_net_data(cls, interface: str, reference_instance):
        ...
        try:
            ...
            return {
                ...
                "today_uploaded": "小于 1 MB",
                "today_downloaded": "小于 1 MB",
                "session_uploaded": "小于 1 MB",
                "session_downloaded": "小于 1 MB",
                "session_duration": "刚刚",
                "alltime_uploaded": "小于 1 MB",
                "alltime_downloaded": "小于 1 MB",
            }
    ...
    def _update_internet_info_in_menu(self):
        ...
        if "internet-info" in self.menu_labels:
            try:
                ...
                status_text = "已连接网络" if self._is_internet_connected else "网络已断开"
    ...
    def _toggle_menu(self):
        ...
        if self._menu["show_interface_name"]:
            interface_label = QLabel(f"网络接口：{self._interface.capitalize()}")
            ...
            try:
                if data is None:
                    if (
                        ...
                        data = ("0", "0", "小于 1 MB", "小于 1 MB", "小于 1 MB", "小于 1 MB", "小于 1 MB", "小于 1 MB", "刚刚")
                ...
                label_updates = {
                    "session-upload": ("上传：", session_uploaded),
                    "session-download": ("下载：", session_downloaded),
                    "session-duration": ("时长：", session_duration),
                    "today-upload": ("上传：", today_uploaded),
                    "today-download": ("下载：", today_downloaded),
                    "alltime-upload": ("上传：", alltime_uploaded),
                    "alltime-download": ("下载：", alltime_downloaded),
                }
