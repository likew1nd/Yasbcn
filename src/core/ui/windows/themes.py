...
class ThemeCard(QFrame):
    ...
    def init_ui(self):
        ...
        author = QLabel(
            f"作者 <a style=\"color:#239af5;font-weight:600;text-decoration:none\" href='{homepage}'>{self.theme_data['author']}</a>"
        )
    ...
    def install_theme(self):
        ...
        self.dialog.setWindowTitle("安装主题")
        ...
        confirmation_message = QLabel(
            f"确定要安装主题 <b>{self.theme_data['name']}</b> 吗？<br>"
            f"这将覆盖当前的配置和样式文件。<br>"
            f"注意：部分主题需要额外字体。<br>"
        )
        ...
        self.yes_button = QPushButton("安装")
        ...
        if self.dialog.exec() == QDialog.DialogCode.Accepted:
            try:
                ...
                if not downloaded:
                    raise last_error if last_error else RuntimeError("下载主题文件失败")
                ...
                QMessageBox.critical(self, "错误", f"安装主题失败：{str(e)}")


class ThemeViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("YASB 主题库")
        icon_path = os.path.join(SCRIPT_PATH, "assets", "images", "app_icon.png")
        icon = QIcon(icon_path)
    ...
    def init_ui(self):
        ...
        # Create backup_info
        self.backup_info = QLabel("在安装新主题前，请备份当前配置文件。")
        ...
        self.load_more_button = QPushButton("加载更多主题")
    ...
    def check_loading_complete(self):
        if self.themes_loaded and self.timer_elapsed:
            if hasattr(self, "load_error_message"):
                self.placeholder_label.setText("加载主题失败。")
                self.placeholder_label.setFont(QFont("Segoe UI", 14))
                QMessageBox.critical(self, "错误", f"加载主题出错：{self.load_error_message}")
            else:
    ...
    def on_fade_out_complete(self):
        ...
        self.loaded_count = 0
        self.load_more_button.setText("加载更多主题")
        if self.theme_items:
    ...
    def display_next_batch(self):
        ...
        if remaining > 0:
            ...
            self.load_more_button.setText(f"再加载 {next_batch} 个主题")
    ...
    def backup_config(self):
        ...
        try:
            ...
            if backup_ok:
                ...
                QMessageBox.critical(self, "错误", "备份失败：缺少备份文件。")
                return

            QTimer.singleShot(
                2000, lambda: (self.backup_button.setText("备份"), self.backup_button.setStyleSheet(original_style))
            )
    ...
    def restore_config(self):
        ...
        try:
            if not os.path.exists(backup_config_path) or not os.path.exists(backup_styles_path):
                ...
                QMessageBox.warning(self, "错误", "恢复失败：缺少备份文件。")
                return
