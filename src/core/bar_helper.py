...
class BarContextMenu:
    ...
    def show(self, position):
        ...
        # Bar info
        bar_info = self._menu.addAction(f"项目：{self._bar_name}")
        ...
        reload_action.triggered.connect(partial(reload_application, "正在从右键菜单重新加载栏..."))
    ...
    def _populate_widgets_menu(self, widgets_menu):
        if not any(self._widgets.get(layout) for layout in ["left", "center", "right"]):
            no_widgets = widgets_menu.addAction("没有活动的小组件")
            no_widgets.setEnabled(False)
            return

        for i, layout_type in enumerate(["left", "center", "right"]):
            # Layout header
            layout_header = widgets_menu.addAction(f"{layout_type.title()} 布局")
    ...
    def _take_screenshot(self):
        ...
        try:
            ...
            if screen is None:
                logging.error("Failed to take screenshot: no screen available")
                return
            screen_geometry = screen.geometry()
            ...
            bar_top = bar_geometry.y() - screen_geometry.y()
            bar_bottom = bar_top + bar_geometry.height()

            if bar_alignment["position"] == "top":
                # For top bar: start from top of this screen and extend to bar bottom + padding
                screenshot_x = 0
                screenshot_y = 0
                screenshot_width = screen_geometry.width()
                screenshot_height = min(screen_geometry.height(), bar_bottom + padding_area)
            else:
                # For bottom bar: start from bar top - padding and extend to screen bottom
                screenshot_x = 0
                screenshot_y = max(0, bar_top - padding_top)
                screenshot_width = screen_geometry.width()
                screenshot_height = screen_geometry.height() - screenshot_y

            if screenshot_width <= 0 or screenshot_height <= 0:
                logging.error("Failed to take screenshot: invalid capture size")
                return
            ...
            if screenshot.isNull():
                logging.error("Failed to take screenshot: empty capture")
            elif not screenshot.save(filepath, "PNG"):
