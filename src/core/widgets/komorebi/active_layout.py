...
layout_display_names = {
    ...
    "vertical_stack": "垂直堆叠",
    "VerticalStack": "垂直堆叠",
    "horizontal_stack": "水平堆叠",
    "HorizontalStack": "水平堆叠",
    "ultrawide_vertical_stack": "超宽垂直堆叠",
    "UltrawideVerticalStack": "超宽垂直堆叠",
    "right_main_vertical_stack": "右主垂直堆叠",
    "RightMainVerticalStack": "右主垂直堆叠",
}
...
class ActiveLayoutWidget(BaseWidget):
    ...
    def _show_layout_menu(self):
        ...
        for layout in self._layouts_config:
            ...
            text = layout_display_names.get(layout, layout.replace("_", " ").title())
        ...
        toggle_icons = {
            "切换平铺": self._layout_icons["tiling"],
            "切换单窗": self._layout_icons["monocle"],
            "切换暂停": self._layout_icons["paused"],
        }
        toggle_actions = [
            ("切换平铺", lambda: self._komorebic.toggle("tiling")),
            ("切换单窗", lambda: self._komorebic.toggle("monocle")),
    ...
    def _get_layout_label_info(self):
        if self._komorebi_state.get("is_paused", False):
            ...
            raw_name = self._focused_workspace["layout"]["Default"]
            layout_name = layout_display_names.get(raw_name, raw_name)
            layout_icon = self._layout_icons.get(layout_snake_case[raw_name], "未知布局")
